...
import com.example.dictionaryapp.utils.Resource as CustomResult
...
class MainViewModel @Inject constructor(
    ...
    private suspend fun loadWordResult() {
        dictionaryRepository.getWordResult(mainState.value.searchWord).collect { result ->
            when (result) {
                ...
                is CustomResult.Success -> {
                    result.data?.let { wordItem ->
                        _mainState.update {
                            it.copy(wordItem = wordItem, isLoading = false)
                        }
                    }
                }
            }
